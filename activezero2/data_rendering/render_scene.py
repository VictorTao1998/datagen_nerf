...
def render_scene(
    ...
    #table_pose_np[4] = table_pose_np[4]+0.2
    ...
    if rand_pattern:
        ...
        fov = 1.6
        alight = scene.add_active_light(
            pose=Pose([0, 0, 0]),#pose=Pose([0.4, 0, 0.8]),
            # pose=Pose(cam_mount.get_pose().p, apos),
            color=[0, 0, 0],
            fov=fov,
    ...
    for view_id in range(num_views):
        ...
        if fixed_angle:
            #print(cam_extrinsic_list[view_id],table_pose.to_transformation_matrix())
            #cam_extrinsic_list[view_id][0,3] = cam_extrinsic_list[view_id][0,3] + 0.5
            #cam_extrinsic_list[view_id][:3,:3] = np.identity(3)
            #print(cam_extrinsic_list[view_id])
            #assert 1==0
            #print(view_id, num_views)
            #assert 1==0
        ...
        if view_id == 0 and not fixed_angle:
            # reproduce IJRR
